				...
				<div class="col-xs-12">
					<div class="box">
						<div class="box-header">
							<h3 class="box-title"><i class="fa fa-server"></i> 主机列表</h3>
							<div class="box-tools">
							</div>
						</div>
						<div class="box-body">
		...
		viewModel = {
				...
				const options = {
					...
					columns:[{
						...
						'data':function(row, type, val, meta) {
							return '<a target="_blank" class="btn btn-link text-info" href="https://'+row.ip+'">' + row.ip + '</a>';
						},
						'orderable': false
					},{
						'data':'port',
			...
			checkServer: function() {
				$('tbody').find('tr').each(function(idx, el) {
					...
					$.get('${ctx}/devops/serverHost/checkServerState/' + id, function(data) {
						if (data) {
							$(el).find('td').eq(7).html('<span class="label label-success">运行中</span>');
							$(el).find('td').eq(9).find('button').last().find('i').removeClass('fa-spin fa-spinner');
							$(el).find('td').eq(9).find('button').last().find('i').addClass('fa-pause');
							$(el).find('td').eq(9).find('button').last().attr({'title': '关闭'});
						} else {
							$(el).find('td').eq(7).html('<span class="label label-danger">无法访问</span>');
							$(el).find('td').eq(9).find('button').last().find('i').removeClass('fa-spin fa-spinner');
							$(el).find('td').eq(9).find('button').last().find('i').addClass('fa-play');
							$(el).find('td').eq(9).find('button').last().attr({'title': '启动'});
						}
					});
				});
			},
