...
<script>
    ...
    function search() {
        ...
        $.get("${ctx}/dlyy/door/getRecordList", {"startDate" : startDate, "endDate": endDate}, function(data) {
            /*var myViewModel = {
                records: ko.observableArray(data)
            };
            ko.applyBindings(myViewModel);*/
            ...
            for (let i = 0; i < dayList.length; i++) {
                ...
                for (let j = 1; j < columnLength; j++) {
                    ...
                    for (let k = 0; k < recordList.length; k++) {
                        ...
                        if (record.recordDate === day && record.personName === personName) {
                            let onAm = record.onDutyAm;
                            let offAm = record.offDutyAm;
                            let onPm = record.onDutyPm;
                            let offPm = record.offDutyPm;
                            let content = '';
                            if (onAm === '-' && offAm === '-' && onPm === '-' && offPm === '-') {
                                content = '';
                            } else {
                                content = '上午：<strong>' + onAm + '</strong> / <strong>' + offAm + '</strong><br>下午：<strong>' + onPm + '</strong> / <strong>' + offPm +'</strong>';
                            }
